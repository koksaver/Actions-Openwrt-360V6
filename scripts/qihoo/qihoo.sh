...
# '修改默认主机名'
sed -i 's/Portable/Qihoo 360V6/g' package/addition/default-settings/files/99-default-settings
...
#　编译的固件文件名添加日期
sed -i 's/IMG_PREFIX:=$(VERSION_DIST_SANITIZED)/IMG_PREFIX:=Qihoo360V6-$(shell TZ=UTC-8 date "+%Y%m%d" "%H:%M:%S)-$(VERSION_DIST_SANITIZED)/g' include/image.mk
